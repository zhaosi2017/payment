...
$().ready(function () {
    ...
    $.validator.setDefaults({
        ...
        submitHandler: function() {
            ...
            //没作任何改动
            if((preData.name == $("#primary_key").val()) && (preData['superior_company_name']==$("#source_superior").val())){
                layer.msg('未作任何修改');
                return;
            }
            ...
            if(preData.name!= $ajaxForm.find('input[name="name"]').attr("data-name-check")) {
                ...
                $.post("", preData).done(function (r) {
                    if (r.code == 0) {
                        ...
                        $("#primary_key").val($ajaxForm.find('input[name="name"]').val());
                        $("#source_superior").val($ajaxForm.find('select[name="superior_company_name"]').val());
